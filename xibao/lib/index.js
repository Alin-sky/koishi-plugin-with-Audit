...
exports.Config = koishi_1.Schema.object({
    ...
    xibao: koishi_1.Schema.object({
        fontFamily: koishi_1.Schema.string().default('"HarmonyOS Sans SC", "Source Han Sans CN", sans-serif')
            .description('字体（参照 CSS 中的 [font-family](https://developer.mozilla.org/zh-CN/docs/Web/CSS/font-family) ）'),
        maxFontSize: koishi_1.Schema.number().min(1).default(80).description('最大字体大小（px）'),
        minFontSize: koishi_1.Schema.number().min(1).default(38).description('最小字体大小（px）'),
        offsetWidth: koishi_1.Schema.number().min(1).default(900)
            .description('单行最大宽度（px），任意一行文本达到此宽度后会缩小字体以尽可能不超出此宽度，直到字体大小等于`minFontSize`'),
    }).description('喜报配置'),
    beibao: koishi_1.Schema.object({
        fontFamily: koishi_1.Schema.string().default('"HarmonyOS Sans SC", "Source Han Sans CN", sans-serif')
            .description('字体（参照 CSS 中的 [font-family](https://developer.mozilla.org/zh-CN/docs/Web/CSS/font-family) ）'),
        maxFontSize: koishi_1.Schema.number().min(1).default(90).description('最大字体大小（px）'),
        minFontSize: koishi_1.Schema.number().min(1).default(38).description('最小字体大小（px）'),
        offsetWidth: koishi_1.Schema.number().min(1).default(900)
            .description('单行最大宽度（px），任意一行文本达到此宽度后会缩小字体以尽可能不超出此宽度，直到字体大小等于`minFontSize`'),
    }).description('悲报配置'),
    advanced: koishi_1.Schema.object({
        importCSS: koishi_1.Schema.union([
            koishi_1.Schema.const('https://gitee.com/ifrank/harmonyos-fonts/raw/main/css/harmonyos_sans_sc.css').description('国内源（Gitee）'),
            koishi_1.Schema.const('https://raw.githubusercontent.com/ifrvn/harmonyos-fonts/main/css/harmonyos_sans_sc.css').description('国外源（GitHub）'),
            koishi_1.Schema.object({
                custom: koishi_1.Schema.string().role('textarea')
...
async function apply(ctx, config) {
    ...
    ctx.command('喜报 <text:text>', '生成一张喜报')
        ...
        .action(async (_, text) => {
        if (!text)
            return '请在指令空格后输入内容，具体使用方式请查看帮助信息';
        //审核
        ...
        //
        const img = (0, fs_1.readFileSync)(path_1.default.resolve(__dirname, './xibao.jpg'));
    ...
    ctx.command('悲报 <text:text>', '生成一张悲报')
        ...
        //
        const img = (0, fs_1.readFileSync)(path_1.default.resolve(__dirname, './beibao.jpg'));
        return await ctx.puppeteer.render(html({
            text,
